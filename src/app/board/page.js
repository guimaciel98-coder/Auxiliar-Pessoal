            ...
            {!isDay && (
              <>
                {activeProject !== "pessoal" && columnsToShow.map(client => {
                  const ct = sortTasksBoard(getTasksForClient(client.cf_value));
                ...
                {(activeProject === "pessoal" || unassignedTasks.length > 0) && (
                  <SortableContext items={unassignedTasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
                    <div className={styles.column}>
                      ...
                      <div className={styles.taskSlot}>
                        {sortTasksBoard(unassignedTasks).map(t => <SortableTaskCard key={t.id} t={t} columnId="unassigned" onComplete={handleComplete} onReschedule={handleReschedule} onEdit={setEditTask} showSection={false} />)}
                      </div>
